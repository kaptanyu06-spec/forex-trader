...
**ผลคัดคู่เงินเพิ่ม (2026-07-18 — ผู้ใช้อยากได้ไม้ถี่ขึ้น ปฏิเสธโหมด scalping ไปแล้วด้วยเหตุผลต้นทุนสเปรด)**: backtest v2/1h/1ปี 4 คู่ใหม่ — NZDUSD ตกชัด (PF 0.5, -12.9%) | USDCAD/EURJPY/GBPJPY: PF 1.29/1.14/1.14 ที่สเปรด 1 pip แต่พอใช้สเปรดสมจริง (1.8/1.8/2.5) เหลือ PF 1.09/1.12/1.10 = ก้ำกึ่งเกินไป **ยังไม่เพิ่มคู่ไหน** | แผนต่อ: วันจันทร์ตลาดเปิด เช็คสเปรดจริงจาก Capital.com API (get_market) — ถ้าคู่ไหนสเปรดจริงถูกกว่าที่ทดสอบจน PF กลับมา >~1.15 ค่อยพิจารณาเพิ่ม | ข้อควรระวังถ้าเพิ่มคู่ cross (EURJPY/GBPJPY): calc_units/calc_size ใน broker ทั้งสองยังไม่รองรับคู่ที่ไม่มี USD (ต้องเขียนแปลงค่าเงิน quote ก่อน)
...
**อัปเดต 2026-07-19**: ทดสอบยิง Capital.com API จริงครั้งแรก **ผ่าน** (login + อ่านยอดบัญชี demo $1,000 + อ่านราคาได้ครบ 5 คู่) — ระบบพร้อมส่งออเดอร์ demo อัตโนมัติเมื่อมีสัญญาณ | สร้าง `check_spreads.py` สำหรับงานเช็คสเปรดวันจันทร์: วัดสเปรดจริงหลายรอบ (นับเฉพาะตอน TRADEABLE) เทียบกับที่ backtest ใช้ แล้วรัน backtest v2/1h คู่ตัวเลือก (USDCAD/EURJPY/GBPJPY) ด้วยสเปรดจริง ตัดสินตามเกณฑ์ PF >= 1.15 อัตโนมัติ — **วิธีรัน: `python3 check_spreads.py` ตอนตลาดเปิด แนะนำจันทร์ช่วงบ่าย-ค่ำเวลาไทย (ตลาดลอนดอนเปิด สเปรดแคบสุด)** | หมายเหตุ: ราคาช่วงตลาดปิดสุดสัปดาห์เป็นราคาแช่ สเปรดถ่างผิดปกติ (เช่น GBPUSD 11 pip) ใช้ตัดสินไม่ได้ สคริปต์จึงกรองทิ้งให้แล้ว

**แดชบอร์ดขึ้นคลาวด์ (2026-07-19)**: ผู้ใช้ตัดสินใจ**เปลี่ยน repo เป็น public** (ยืนยันแล้ว) เพื่อตัดปัญหา authorize private repo กับ Streamlit ที่ติดมา 2 วัน — ตรวจก่อนเปลี่ยนแล้วว่าไม่มีคีย์ลับในโค้ด/ประวัติ commit (secrets_local.py ไม่เคยถูก commit, คีย์ทั้งหมดอยู่ใน GitHub Secrets) | ค่า deploy ที่ถูกต้อง: repo `kaptanyu06-spec/forex-trader`, branch **`main`** (ไม่ใช่ master — ผู้ใช้เคยกรอกผิด), main file `dashboard.py` | **Deploy สำเร็จ 2026-07-19 — URL จริง: https://forex-trader-ndf39u9xknyehcjappl9elz.streamlit.app** (URL แรกที่จดไว้ก่อนหน้า dqto6... ตายแล้ว — เป็นของฟอร์มรอบแรกที่ไม่ได้ถูกสร้าง) | เกร็ด: ถ้าแอปเปิดจากเครื่องอื่นแล้วเจอหน้าล็อกอิน ให้ตั้ง Sharing เป็น "This app is public and searchable" ใน app Settings

**งานค้างเดิม (2026-07-18 กลางคืน — จบแล้วด้วยการเปลี่ยน public ข้างบน)**: Deploy แดชบอร์ดขึ้น Streamlit Community Cloud — ฝั่ง repo พร้อมหมดแล้ว (analysis_latest.json commit ทุกรอบ, บั๊ก dashboard แก้แล้ว, ทดสอบ workflow ผ่าน) เหลือแค่ผู้ใช้กด deploy ที่ share.streamlit.io แต่ติดขั้นให้สิทธิ์ private repo: ต้องทำ Workspaces (มุมซ้ายบน) -> Connect GitHub account แล้วตามด้วย ชื่อผู้ใช้มุมซ้ายบน -> Settings -> Linked accounts -> Source control -> Connect here -> Authorize streamlit | แผนสำรองถ้ายังติด: เปลี่ยน repo เป็น public (ผู้ใช้ยังไม่ได้ตัดสินใจ — ไม่มีรหัสลับใน repo, คีย์อยู่ใน GitHub Secrets ทั้งหมด)
